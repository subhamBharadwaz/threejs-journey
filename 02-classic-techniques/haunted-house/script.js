...
roof.position.y = 2.5 + 0.75; // 2.5 is the height of the walls and 0.75 is half of the roof's height because the cone's origin is at its center
roof.rotation.y = Math.PI * 0.25; // 1/8 circle = Math.PI * 0.25
houseGroup.add(roof);
...
bush1.scale.setScalar(0.5); // if we have same values instead of set(0.5,0.5,0.5) we can use setScalar(0.5)
...
for (let i = 0; i < 30; i++) {
  const angle = Math.random() * Math.PI * 2; // for getting random angles of full circle (Math.PI * 2)
  const radius = 3 + Math.random() * 4;
  const x = Math.sin(angle) * radius;
  const z = Math.cos(angle) * radius; //named this z instead of y because x and y are just the generic names for the axes in 2D space, but in our case, we want to move the graves along the x and z axes.
 ...
 */
// Ambient light
//* Having dimmed ambientLight allows the user to enjoy surfaces in the shade and mimics the light's bounce of directionalLight
...
const tick = () => {
  ...
  // Door light flick
  const slowOscillation = Math.sin(elapsedTime * 0.5) * 0.3;

  // Add some random noise for subtle flickering
  const randomFlicker = (Math.random() * 2 - 1) * 0.5;

  // Combine base intensity, oscillation, and random flicker
  doorLight.intensity = 3 + slowOscillation + randomFlicker;
